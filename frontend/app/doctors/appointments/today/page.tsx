...
export default function DoctorTodayAppointmentsPage() {
  ...
  async function load() {
    ...
    try {
      const d = todayStr();
      const res = await fetch(`/api/doctors/appointments?status=CONFIRMED&date=${d}`, {
        cache: "no-store",
      });
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto max-w-5xl px-6 py-10">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold">นัดวันนี้ (CONFIRMED)</h1>
            <p className="mt-1 text-sm text-white/70">รายการนัดที่ยืนยันแล้วเฉพาะ “วันนี้”</p>
          </div>
          <button
            onClick={load}
        ...
        <div className="mt-6">
          {loading ? (
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/70">
              วันนี้ยังไม่มีนัดที่ยืนยันแล้ว
            </div>
          ) : (
              ...
              {items.map((a) => (
                <div key={a.appointment_id} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  ...
                  <div className="mt-2 text-sm text-white/70">
                    ...
                    {" · "}
                    เพศ: <span className="font-semibold text-white">{a.patient?.gender || "-"}</span>
                    {" · "}
                    #{a.appointment_id}
                  </div>
                </div>
