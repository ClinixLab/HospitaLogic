...
export async function GET(req: Request) {
  ...
  // ✅ ชัดเจนกว่า: ถือว่า “จองอยู่” เฉพาะสถานะที่ยัง block เวลา
  // ถ้า enum ของคุณมีแค่ PENDING/CONFIRMED/CANCELLED/COMPLETED ให้ใช้ชุดนี้
  const booked = await prisma.appointment.findMany({
    where: {
      doctor_id,
      date: { gte: dayStart, lt: dayEnd },
      status: { in: ["PENDING", "CONFIRMED", "COMPLETED"] }, // <- ปรับตรงนี้
    },
    select: { time: true },
  });
