...
// ✅ ปลอดภัยกว่าการ new Date("YYYY-MM-DDT00:00:00")
  ...
  try {
    ...
    const booked = await prisma.appointment.findMany({
      where: {
        ...
        // ✅ ตัด REJECTED ออกก่อน (ตามระบบคุณมีแค่ 4 สถานะ)
    ...
    const now = new Date();
    const minAllowed = sameLocalYMD(dayStart, now)
      ? Math.max(openMin, next30MinFromNowMinutes(now))
      : openMin;
    ...
    console.error("GET /api/appointments/slots error:", e);
    return NextResponse.json(
      { message: "Internal Server Error", detail: String(e?.message ?? e) },
      { status: 500 }
    );
  }
}
